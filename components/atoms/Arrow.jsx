...
export default function Arrow({ up, left, right, className , handleClick, disable}) {
  ...
  let [hovering, setHovering] = useState(false)
    ...
    return (
      <svg transform={`${left?'rotate(-90)':right?'rotate(90)':up?'':'rotate(180)'}`} style={{ transformOrigin: "center", transformBox: 'fill-box'}} key={`${left?'arrowLeft':right?'arrowRight':up?'arrowUp':'arrowDown'}`}  onMouseEnter={()=>setHovering(true)} onMouseLeave={()=>setHovering(false)} onClick={handleClick} 
      className={'transition-all duration-500 ' + className} viewBox="0 0 26 22" fill="none" xmlns="http://www.w3.org/2000/svg">
