...
<>
      ...
      {cart.content.length >= 1 &&
        <motion.div layout key='filled-cart' onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}
          className={`m-0 px-4 z-[51] absolute flex flex-col justify-between text-center w-screen min-[480px]:w-[420px] top-0 right-0 min-[480px]:right-2 min-[480px]:top-2`}
          ...
          <motion.div variants={childVariants} layout className='flex flex-col transition-all mb-5 p-4 gap-2 bg-black/30 rounded-2xl' style={{ bottom: 0 + 'px' }} >
            <CartItem title />
            {/* <AnimatePresence > */}
              {
                cart.content.map((item) => {
                  // console.log(item)
                  return <CartItem myId={item.id + item.size} key={item.id + item.size} item={item} />
                })
              }
            {/* </AnimatePresence> */}
          </motion.div>
      ...
      {/*  ====================== EMPTY CART ====================== */}
      {cart.content.length < 1 &&
        <motion.div layout key='empty-cart' onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}
          className={`m-0 px-4 z-[51] absolute flex flex-col justify-between items-center text-center w-screen min-[480px]:w-[420px] top-0 right-0 min-[480px]:right-2 min-[480px]:top-2`}
          variants={containerVariants}
          >
