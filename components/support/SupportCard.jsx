...
const {setSupportAmount, updateSupport } = useAppContext().cart
  ...
  return (
    ...
    font-thin text-center font-quick text-sm whitespace-pre-wrap  text-primary'>

      <p className='text-xs'>
        {`From `}<span className='font-medium'>€{amount}</span>
        {` support you will receive:`}
      </p>
      <div className='border-y-2 border-primary font-sans whitespace-nowrap inline-flex text-primary font-semibold uppercase tracking-max text-sm lg:text-base py-1'>
        <p> {title} </p>
      </div>
