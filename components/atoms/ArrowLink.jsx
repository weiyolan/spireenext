...
const ArrowLink = ({ text, to, ext, inherit, inText, tabIndex }) => {
  ...
  if (ext) {
    return (
      ...
      <div className={`inline-flex ml-5 items-center ${inText ? 'mt-0' : 'mt-4'} relative`}
  ...
  return (
    ...
    <span className='inline-flex ml-5 items-center mt-4 relative'
      onMouseEnter={() => setHovering(true)}
      onMouseLeave={() => setHovering(false)}>
      <Link tabIndex={tabIndex} className={` ${inherit ? 'text-inherit font-medium' : 'text-white font-light font-sans'} min-[460px]:whitespace-pre-wrap sm:whitespace-nowrap text-sm  `}
  ...
  return (
    <span className='block duration-300 relative w-10 h-5 overflow-hidden '>
      <span className={`absolute duration-300 top-1/2 left-0 -translate-y-1/2 ${hovering ? 'translate-x-1/4 scale-125 ' : '-translate-x-[100%] opacity-0'}`}>
        <BsArrowRightShort className={`${inherit ? 'text-inherit fill-inherit' : 'fill-white'} text-xl`} />
      </span>
